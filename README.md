...
`release/sidetab-lite-0.4.0.zip` 用于上传 Chrome Web Store 或传递构建产物。Chrome 通常不能直接双击 ZIP 安装扩展；本地测试请加载已解压的 `dist` 目录。
...
标签页右键菜单支持复制、固定或取消固定，并可将当前 HTTP/HTTPS 页面直接设为快捷网站。新快捷网站保存页面完整地址，名称优先使用标签标题。
