...
SideTab Lite 是一个 Chrome 侧边栏标签页管理扩展。它支持切换、关闭、固定和拖动排序当前窗口的标签页，提供历史记录搜索及可选的快捷网站入口。快捷入口默认关闭，设置保存在 Chrome 本地存储中。
...
`release/sidetab-lite-0.5.0.zip` 用于上传 Chrome Web Store 或传递构建产物。Chrome 通常不能直接双击 ZIP 安装扩展；本地测试请加载已解压的 `dist` 目录。
...
标签页右键菜单支持复制、固定或取消固定、设为快捷网站，以及“关闭下方标签页”。“关闭下方标签页”只关闭当前标签之后的普通标签，保留当前标签和所有固定标签。新快捷网站保存页面完整地址，名称优先使用标签标题。
...
扩展只申请 `sidePanel`、`tabs`、`storage` 和 `history` 权限，不申请主机权限，也不注入 content script。`history` 仅用于用户操作底部搜索框时，在本地查询并展示 Chrome 历史记录；查询结果不持久化，也不会被上传。扩展不收集、不出售，也不向开发者服务传输用户数据。
