...
SideTab Lite 是一个 Chrome 侧边栏标签页管理扩展。它支持切换、关闭、固定、分组和拖动排序当前窗口的标签页，也可通过鼠标中键快速关闭标签页；同时提供历史记录搜索及可选的快捷网站入口。快捷入口默认关闭，设置保存在 Chrome 本地存储中。
...
底部搜索框用于搜索 Chrome 本地历史记录，不会过滤当前标签页。聚焦搜索框时默认显示最近 20 条记录；输入内容后最多显示 20 条匹配结果，结果框向上展开。选择结果会在新的活动标签页中打开。

标签页右键菜单支持复制、固定或取消固定、设为快捷网站、添加到分组、从分组中移除，以及“关闭下方标签页”。“添加到分组”的二级菜单可选择已有分组或“新建分组”；分组标题可展开或折叠，折叠状态会同步到 Chrome。鼠标中键点击标签页会直接关闭该标签页。“关闭下方标签页”只关闭当前标签之后的普通标签，保留当前标签和所有固定标签。新快捷网站保存页面完整地址，名称优先使用标签标题。
...
扩展只申请 `sidePanel`、`tabs`、`tabGroups`、`storage` 和 `history` 权限，不申请主机权限，也不注入 content script。`tabGroups` 仅用于读取和管理当前窗口的 Chrome 原生标签组；`history` 仅用于用户操作底部搜索框时，在本地查询并展示 Chrome 历史记录。查询结果不持久化，也不会被上传。扩展不收集、不出售，也不向开发者服务传输用户数据。
