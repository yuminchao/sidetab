...
- [ ] 空标签列表时加号仍可见、可点击。
- [ ] 右键菜单包含四项：复制标签页、根据当前状态显示“固定标签”或“取消固定”、设为快捷网站、关闭下方标签页；鼠标悬停、键盘焦点和按下状态清晰，`Shift+F10`、方向键和 `Esc` 操作正常。
...
- [ ] 权限精确为 `sidePanel`、`tabs`、`storage`、`history`，没有 host permission 和 content script。
...
- [ ] 权限理由分别说明：侧边栏展示、当前窗口标签管理、本地保存快捷入口和字号设置，以及本地搜索 Chrome 历史记录。
...
- [ ] 标签右键菜单使用微软雅黑字体栈，180px 宽度下包含四项的菜单不溢出；浅色、深色主题下拖动插入线均清晰可辨。
